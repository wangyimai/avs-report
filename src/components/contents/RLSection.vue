<template>
  ...
  <article class="message is-link">
    <div id="强化决策"  class="message-body">
      ...
      <br/>
      <br/>
      You等人（2018）提出马尔可夫决策过程（MDP）对自动驾驶汽车与周围车辆的相互作用进行建模。MDP允许自动驾驶汽车在超车或尾随另一辆车时，根据道路结构做出适当的决定。这种方法仍需要在不同的场景中进行更多的测试来进行验证。
      <br/>
      <br/>
      Hoel等人（2019）提出了一种策略决策方法，该方法结合了蒙特卡罗树搜索（MCTS）和强化学习算法，在两种高速公路驾驶情况下控制自动驾驶汽车（包括连续高速公路驾驶和退出高速公路）。他们的提案在模拟高速公路环境中进行了测试，初步结果显示了RL和MCTS相结合的有效性。
      <br/>
      <br/>
      ...
      <br/>
      <br/>
      Okuyama等人（2018）将卷积神经网络和强化学习相结合，在具有车道标志和静态障碍物(例如人、停止的车辆)的模拟环境中训练自动驾驶汽车。CNN利用由自动驾驶汽车的前置摄像头拍摄的图像，以提取道路的主要状态特征。这些特征被反馈给DQN以预测下一步的行动。他们的方法的不足之处是没有考虑动态障碍。
      <br/>
      <br/>
      ...
      <br/>
      <br/>
      Chen等人（2019a）提出了一种在车辆密集的环形交叉口上控制自动驾驶汽车的方法。他们的方法从传感器获取周围每个物体的信息（例如速度和位置）；该方法使用DQN预测正确的动作。他们在Carla模拟器中模拟了他们的算法，模拟结果显示出该方法在解决环形交叉口驾驶情况的积极作用。
      <br/>
      <br/>
      Wolf等人（2017）使用DQN在模拟环境中训练自动驾驶汽车。利用摄像头，自动驾驶汽车识别道路并采取转弯动作（例如，左、半左、直、半右和右）。这种训练环境需要考虑到静态和动态的障碍。
      <br/>
      <br/>
      ...
      <br/>
      <br/>
      Hu等人（2019）提出了一种基于强化学习的方法，在两个场景的融合中控制自动驾驶汽车的决策方法。他们的方案在所有测试场景中实现了零碰撞。这种方法在机动性较高的实际交通中并不能保证安全。
      <br/>
      <br/>
      Wang等人（2020）提出了一种在复杂高速道路中生成驾驶策略的方法。基于反馈给DQN的传感器信息（例如，车辆位置和速度）来学习如何为复杂高速公路中的每辆车辆做出决策（加速、减速、保持车道、右转和左转）。模拟结果表明该方法提高了道路的安全性。
      <br/>
      <br/>
  ...
  <article class="message is-link">
    <div id="强化社会行为"  class="message-body">
      ...
      <br/>
      <br/>
      Nasernejad等人（2021）将交叉口行人和自动驾驶汽车之间的相互作用建模为MDP问题。该方法能够让自动驾驶汽车根据行人在环境中的行为来避开行人。然而，该方法局限性在于仅仅作用于行人和车辆的相互作用，没有考虑到其他相互作用(例如自行车)。
      <br/>
      <br/>
