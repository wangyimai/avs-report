<template>
  ...
  <article class="message is-link">
    <div  class="message-body">
      ...
      <br/>
      <br/>
      在过去二十年中，场景理解领域使用有效的分布式语言技术取得了重大进展。这使得不同的传感器（如激光雷达、摄像机和雷达）为AVS提供有关驾驶环境的关键和精确信息成为可能。深度CNN在实时图像分类和检测方面已经有些显著的结果（Fernandes et al., 2021）。如图4所示，场景理解中DL技术的使用为车道、交通灯、行人、交叉线、交通标志等的检测提供了可操作的信息。Cordts et al. (2016)。
      <br/>
      <br/>
  ...
  <div class='pic'>
      <img id="场景理解图片" src='../../assets/mbjc.webp' width="900" height="1000">
      <p>图：场景理解</p>
  </div>
  <article class="message is-link">
    <div id="深度道路场景"  class="message-body">
      ...
      <br/>
      <br/>
      准确识别和提取主要道路信息对于AVS识别道路要素（例如路面、沟渠、护栏和栅栏）至关重要。由于DL的强大功能，可以实现高精度的道路要素识别。例如，Balado et al. (2019)使用移动激光扫描（MLS）获得的信息来识别道路环境的主要元素。他们的提议基于点网（Qi et al., 2017）和语义分割（SS）方法来识别道路元素，这为城市地区的AVS提供了良好的前景。
      <br/>
      <br/>
      识别道路的几何结构可以为AVS提供有关其环境的更多信息。Laddha et al.(2016)用基于监督和非监督学习的混合算法，使用CNN识别道路几何形状。虽然该算法减少了人为标记的工作量和使训练更具可扩展性，但仍需要在具有关键天气条件的不同场景中对其进行评估。
      <br/>
      <br/>
      全面了解周围环境和道路区域（例如遮挡、3D几何形状和道路拓扑类型）有助于自主驾驶的实际应用。
      Yan et al. (2020)开发了一种基于激光雷达数据的多任务道路感知网络（LMRoadNet）方法。LMRoadNet旨在检测和估计道路测量值，如道路的长度和形状，以识别其拓扑结构。
      <br/>
      <br/>
      道路转角是识别具有挑战性的区域之一。Bolte et al. (2019)的一项研究引入了道路转角的形式定义，并AVS摄像机的视频信号中进行检测。因此，基于CNN的系统使AVS能够在关键情况下感知更好的道路。
      <br/>
      <br/>
      Tümen and Ergen (2020)开发了一种图像处理方法和基于DL的方法，用于CNN检测公路上的十字路口、分隔带和人行横道。这种检测对于改善道路安全至关重要是因为这些地方的事故发生率很高。
      <br/>
      <br/>
      Di et al. (2017)研究了交通场景理解问题，以从具有照明条件的图像中识别道路物体。他们考虑了基于CNN的密集对应迁移学习方法，以提取城市环境图像的深度表示。但是这项研究的问题是没有考虑到不同和复杂的驾驶场景。
      <br/>
      <br/>
      交通标志的成功检测和分类是全自动驾驶汽车要克服的主要挑战之一。例如，Sajjad et al. (2020)开发了一种检测和避开障碍物的方法。该模型基于视觉传感器识别各种交通标志同时使用超声波传感器避开障碍物。这项研究的意义是仅使用单目视觉传感器来促进车辆感知。
    </div>
  </article>
  ...
  <article class="message is-link">
    <div id="深度目标检测"  class="message-body">
      ...
      <br/>
      <br/>
      准确、实时地检测周围物体，包括其他道路使用者，对于AVS来说非常重要。Wang et al. (2020b) 提出了一种基于DL技术的端到端三维目标检测方法。这项技术使用CNN和融合网络（FoFNet）来预测边界框和区分对象例如汽车、行人和骑自行车的人的类别。
      <br/>
      <br/>
      Prabhakar et al. (2017)提出了一种通过考虑天气条件来提高检测效率的方法。他们使用区域卷积神经网络（R-CNN）来识别和分类障碍物，如车辆、行人和动物。雨天时，他们使用在每个检测到的对象顶部带有类名的边界框。但是现在的问题是需要对其他道路图像数据集进行更多测试才能评估该模型的准确性和适用性。
      <br/>
      <br/>
      除了目标检测之外，还需要估计其他关键参数（例如距离），以帮助实现自动驾驶。Chen et al. (2018b)提出多任务组合策略（CPMTL）算法，该算法旨在通过在检测过程中估计车辆与其他道路使用者之间的距离来改进检测。
      <br/>
      <br/>
      Li et al. (2021)解决了夜间使用CNN进行检测的问题，以改进微光图像识别。他们的提案可以帮助AVS识别道路，同时他们在没有路灯的农村地区进行了更深入的研究。
      <br/>
      <br/>
      Dinh et al. (2020)提出了一种基于CNN的方法，用于更好地检测城市交通中的目标；他们假设AVS配备了两个焦距不同的摄像机。研究表明他们的提议允许检测小型物体（例如，小型和遥远的车辆）。
      <br/>
      <br/>
      现有的几种方法依赖视频进行目标检测。比如说随着时间的推移在连续的帧中分类不同对象。然而，这种方法没有考虑导致信息丢失的图像帧之间的空间和时间相关性。为了填补这一空白Liu et al.(2020) 基于运动辅助特征校准网络（MFCN）在遮挡和截断两种情况下识别车辆。与Ren et al. (2017), Song et al. (2018b), and Zhang et al. (2017)相比，他们的提案在不同的物体外观下检测精度更好。
      <br/>
      <br/>
      由于来自传感器的数据量巨大，在训练过程中可以通过减少数据量来提高检测模型的性能，比如说只使用重要数据可以缩短基于DL的方法的训练时间。Das et al., (2020）提出驾驶期间尤其是在紧急情况下的时间至关重要，因此检测必须是实时的。
      <br/>
      <br/>
      三维物体检测揭示了更准确的物体大小和位置信息。自主驾驶的许多重要方面，如运动规划和车辆控制，通常需要忠实地表示车辆周围的三维空间。现有的3D物体检测贡献在使用传感器进行检测的方式上有着根本的不同。一些研究（Ma et al., 2019; Peng et al., 2020）仅使用单目摄像机。例如，Zhang et al. (2020b)提出了一种新的单目框架，用于自动驾驶场景中的三维检测。这项检测的目标是使用3D盒检测和定位对象。
      <br/>
      <br/>
      其他几种方法提出了摄像机图像和激光雷达数据的融合。Chen et al. (2017a)试图通过开发基于多视图三维网络（MV3D）的三维物体检测方法来提高检测性能。他们的方案使用激光雷达点云和RGB图像来预测3D边界盒，这优于仅依赖传感器的方法。Hong et al. (2020) 提出了一种3D检测方法，该方法利用激光雷达点云和RGB图像作为交叉融合网络的输入，他们通过模拟表明这种检测通常仅使用传感器难以检测的对象。
      <br/>
      <br/>
      弱势道路使用者的安全仍然是一个具有挑战性的重大问题。因此，行人的检测至关重要，因为他们被视为主要的弱势道路使用者。在此背景下，Zhao et al. (2020)提出了一种称为行人位置感知网络（P-LPN）的方法；它基于内部级联网络（InCNet）。P-LPN将行人分为两组：在车道上移动的行人和在人行道上移动的行人。它使用区域建议网络（RPN）在语义地图上提供每个行人的位置。然而P-LPN没有考虑行人的意图，这在AVS理解行人的运动的过程中是非常重要的一步。
    </div>
  </article>
  <div class='pic'>
      <img id="深度学习中的场景理解方法比较" src='../../assets/dlzdcjljffbj.png' width="900" height="1000">
      <p>表：深度学习中的场景理解方法</p>
  </div>
  </div>
...
<div id="深度运动规划" >
  <article class="message is-link">
    <div   class="message-body">
      ...
      <br/>
      <br/>
      在场景理解（或环境感知）之后，运动规划是AVS的下一个主要任务，以避免在不同环境中发生碰撞，从而安全平稳地导航。为了处理运动规划问题，传统方法（例如：Dijkstra算法、A-Star算法、State lattice算法）依赖于优化模型。这些算法在计算范围太大时速度缓慢，不适合实时应用。因此，许多研究人员提出了替代方法，例如基于DL的技术。这些方法分析感知信息，以便识别障碍，然后预测正确的动作。这使得AVS可以通过在时间和距离方面找到有效的路径来安全导航。
      <br/>
      <br/>
      我们将运动规划分为两类：运动命令预测和轨迹预测。运动命令预测主要包括计算转向角、加速度和/或制动，而轨迹预测包括计算所有道路参与者（包括ego-vehicle）的短期轨迹及其未来状态。
    </div>
  </article>
  <div class='pic'>
      <img id="深度学习运动规划流程" src='../../assets/dlydghlc.png' width="900" height="1000">
      <p>图：深度学习运动规划流程</p>
  </div>
  <article class="message is-link">
    <div class="message-body">
      ...
      <br/>
      <br/>
      由于交通状况的复杂性，很难建立一个通用的运动规划系统，尤其是实时运动规划系统。事实上，道路使用者的行为仍然不可预测，难以建模。例如，Bai et al. (2018)将CNN和LSTM结合起来提取时空信息，目的是基于真实数据实时预测转向角。Song et al. (2018a)使用从驾驶模拟器收集的数据，使用类似的方法估计转向角。
      <br/>
      <br/>
      Chen et al. (2018a)提出将CNN和LSTM结合起来进行规划决策。他们报告说，使用模拟和真实交通可以使AVS在许多情况下更好地模拟人类驾驶。然而，我们认为，应该更广泛地测试他们的方案，以评估和改进其性能。
      <br/>
      <br/>
      一些贡献（Bai et al., 2018; Song et al., 2018a; Chen et al., 2018a）只关注转向角估计；其他人确实解决了不止一个运动命令。例如，Hu et al. (2020)提出了一种基于CNN和LSTM的深度级联神经网络，以同时预测多个运动指令（即转向角、加速度和制动）。
    </div>
  </article>
  <article class="message is-link">
    <div class="message-body">
      ...
      <br/>
      <br/>
      AVS需要分析传感器收集的数据，以检测障碍物，从而安全导航到目的地。Zhao et al. (2019)提出了卡尔曼学习网络（LKN），以根据摄像机使用DL收集的数据估计车辆轨迹。Banzhaf et al. (2019)提出了一种基于CNN的方法，该方法使用收集的数据预测车辆未来的运动，同时考虑静态障碍物和无规则的道路。因此，在他们的方案中，双向RRT*被用以减少计算时间。
      <br/>
      <br/>
      Grigorescu et al. (2019)提出了一种改进AVS最优路径轨迹预测的方法。他们融合激光雷达和雷达数据来估计最佳轨迹。Cui et al. (2019)提出了一种估计其他车辆和行人轨迹的方法；他们使用CNNs处理来自激光雷达或雷达的数据。Djuric et al. (2020)使用类似的方法预测短期车辆轨迹和道路参与者（例如周围车辆）的未来状态。Deo and Trivedi (2018)提出了CNN和LSTM，以预测各种交通场景中周围车辆的未来轨迹。Zhang et al. (2020c)通过探索人类驾驶员在环境中的体验特征，使用模仿学习。它们利用CNNs处理3D环境信息，并提供周围车辆的状态。
      <br/>
      <br/>
      Leordeanu and Paraicu (2021)开发了一种方法，仅基于视频和最终目的地预测AVS的当前位置和未来轨迹。该方法可以适应不同天气条件下的短期和长期导航。
      <br/>
      <br/>
      Jeong et al. (2020)提出了一种基于LSTM和RNNs的运动预测算法；目标是减少十字路口的交通事故。该算法以周围车辆的状态信息为输入，预测目标状态序列。
      <br/>
      <br/>
      Rehder et al. (2018)使用完全卷积网络（FCNs）操作环境地图，以推断所有可能的步行目的地。仿真结果表明，该系统能够准确预测目的地和轨迹。
    </div>
  </article>
  <div class='pic'>
      <img id="深度学习运动规划方法" src='../../assets/dlydghff.png' width="900" height="1000">
      <p>表：深度学习运动规划方法</p>
  </div>
</div>
  ...
  <div class='pic'>
      <img id="深度学习决策算法比较" src='../../assets/dljcsfbj.png' width="900" height="1000">
      <p>表：深度学习决策算法</p>
  </div>
  ...
  <div class='pic'>
      <img id="深度学习传统控制算法比较" src='../../assets/dlkzsfbj.png' width="900" height="1000">
      <p>表：深度学习传统控制算法</p>
  </div>
  ...
  <div class='pic'>
      <img id="深度学习横向控制算法比较" src='../../assets/dlhxkzsfbj.png' width="900" height="1000">
      <p>表：深度学习横向控制算法</p>
  </div>
  ...
  <div class='pic'>
      <img id="深度学习横向纵向算法比较" src='../../assets/dlzxkzsfbj.png' width="900" height="1000">
      <p>表：深度学习纵向控制算法</p>
  </div>
  ...
  <div class='pic'>
      <img id="深度学习车辆控制算法总结" src='../../assets/dlclkzsfdb.png' width="900" height="1000">
      <p>表：深度学习车辆控制算法总结</p>
  </div>
  <div class='pic'>
      <img id="深度学习车辆控制流程" src='../../assets/dlclkzlc.png' width="600" height="1000">
      <p>图：深度学习车辆控制流程</p>
  </div>
  </div>
  ...
  <div class='pic'>
      <img id="深度学习车辆社会行为算法比较" src='../../assets/dlclshxwsfbj.png' width="900" height="1000">
      <p>表：深度学习车辆社会行为算法</p>
  </div>
  ...
  <article class="message is-link">
    <div id="深度学习车辆通信"  class="message-body">
      ...
      <br/>
      <br/>
      Zhang等人(2020d)使用DNN处理GPS数据，即每辆车的时间戳、纬度和经度信息。这些数据在DNN模型下预测CSI。
    </div>
  </article>
  <div class='pic'>
      <img id="深度学习车辆通信算法比较" src='../../assets/dlcltxsfbj.png' width="900" height="1000">
      <p>表：深度学习车辆通信算法</p>
  </div>
...
import NumberPapers2 from "../charts/NumberPapers2.vue";
